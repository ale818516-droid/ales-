...
-- ==========================================================
-- PESTAÑA Y SECCIÓN SHERIFF (Lógica unificada)
-- ==========================================================
local SheriffTab = Window:Tab({Title = "Sheriff", Icon = "shield"})
local SecSheriff = SheriffTab:Section({Title = "Sheriff Options"})

SecSheriff:Toggle({
    Title = "Auto Shoot [Target Murderer]",
    Default = false,
    Callback = function(state)
        _G.AutoShootSheriff = state
        if state then
            task.spawn(function()
                while _G.AutoShootSheriff do
                    ...
                    if Character and Backpack then
                        ...
                        if Gun and ShootRemote and MyRoot then
                            -- Lógica de detección de Murderer
                            local Murderer = nil
                            ...
                            if Murderer and Murderer.Character and Murderer.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                -- Equipar arma si está en mochila
                                if Gun.Parent == Backpack then Gun.Parent = Character end
                                
                                -- Ejecución (Flick logic)
                                task.defer(function()
                                    local predicted = TargetRoot.CFrame + (TargetRoot.Velocity * 0.125)
                                    ShootRemote:FireServer(MyRoot.CFrame, predicted)
                                    
                                    -- Gestión de re-equipamiento
                    ...
                    task.wait(0.1) -- Loop de alta velocidad
                end
            end)
        end
    end
...
-- ==========================================================
-- TOGGLE CON BOTÓN FLOTANTE PERSONALIZADO
-- ==========================================================
...
SecSheriff:Toggle({
    ...
    Default = false,
    Callback = function(state)
        _G.MobileMurdererButton = state

        local GuiParent = gethui and gethui() or game:GetService("CoreGui")
        local GuiName = "Custom_Murderer_GUI"
        local BtnName = "Custom_Murderer_Btn"
        ...
        if state then
            ...
            -- Crear botón
            local MiBoton = Instance.new("ImageButton")
...
MiBoton.Parent = ScreenGui
            ...
            local Gui = GuiParent:FindFirstChild(GuiName)
            if Gui then
                Gui:Destroy()
            end
        end
